...
import { Loader2, LogOut, Check, Trash2, Menu, Package, Compass, Wand2, Flame, Heart, Sun, Scissors, BookText, Sparkles, X } from 'lucide-react';
...
const DashboardTour = dynamic(
  () => import('../../components/dashboard-tour').then(mod => mod.DashboardTour),
  { ssr: false }
);
...
const TaskItem = ({ task, onToggle, onDelete }: { task: Task; onToggle: (id: string) => void; onDelete: (id: string) => void; }) => {
    ...
    return (
        <div
            key={task.id}
            className={`group flex items-center gap-4 p-3 rounded-lg border-r-4 transition-all duration-300 cursor-pointer ${
                task.completed ? 'bg-brand-primary/10 border-brand-primary' : 'bg-white hover:bg-gray-50 border-transparent'
            }`}
            onClick={() => onToggle(task.id)}
        >
            <div
                className={`flex-shrink-0 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
                task.completed ? 'bg-brand-primary border-brand-primary text-white' : 'border-gray-300'
                }`}
            >
        ...
        <div className="flex flex-col h-full bg-white/80 backdrop-blur-xl">
            <ScrollArea className="flex-1 pt-4">
                <div className="p-4 space-y-4 flex flex-col items-center">
                    <div className="w-full sidebar-menu-area space-y-4">
                    ...
                    <div className="w-full gamification-area">
            ...
            <div className="p-4 border-t flex-shrink-0 add-task-area">
...
export const Dashboard: React.FC<{ profile: UserProfile }> = ({ profile }) => {
    ...
    useEffect(() => {
        const hasSeenWelcome = localStorage.getItem('shaily_has_seen_welcome');
        if (!hasSeenWelcome) {
            setShowWelcome(true);
        }
    }, []);
    ...
    const startTour = () => {
        ...
        setIsTourOpen(true);
        localStorage.setItem('shaily_has_seen_welcome', 'true');
    };
    ...
    return (
        <div className="h-screen w-screen flex bg-gradient-to-br from-brand-cream to-brand-beige" style={{ direction: 'rtl' }}>
            <style jsx global>{`
                .custom-intro-js-tooltip {
                    font-family: 'Vazirmatn', sans-serif !important;
                }
            `}</style>
            <DashboardTour enabled={isTourOpen} onExit={() => setIsTourOpen(false)} />
                ...
                {showWelcome && (
                    ...
                    >
                        <motion.div
                            ...
                            animate={{ scale: 1, y: 0 }}
                            className="bg-white rounded-3xl p-8 max-w-lg w-full text-center relative"
                        >
                            <Button variant="ghost" size="icon" onClick={closeWelcome} className="absolute top-4 right-4 rounded-full">
                                <X className="h-5 w-5" />
                            </Button>
                            <Sparkles className="h-16 w-16 text-brand-primary mx-auto mb-4" />
                            <h2 className="text-3xl font-bold mb-4">به شایلی خوش آمدی, {profile.name}!</h2>
                            <p className="text-gray-600 mb-8">
                                خوشحالیم که اینجا هستی! دوست داری یک تور سریع برای آشنایی با قابلیت‌های داشبورد داشته باشی؟
                            </p>
                            <Button size="lg" onClick={startTour} className="w-full">بله، نشانم بده!</Button>
                        </motion.div>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <div className="flex-1 flex flex-col h-screen">
                ...
                <main className="flex-1 overflow-hidden relative">
                    <div className="absolute top-6 left-6 z-20 hidden md:flex items-center gap-3 user-menu-area">
                        <UserMenu 
                            user={profile} 
                            onLogout={logout}
                            onOpenProfile={() => setProfileModalOpen(true)}
                    ...
                    <div className="h-full ai-chat-area">
                        <AIChat
                            profile={profile}
                            currentTasks={tasks}
